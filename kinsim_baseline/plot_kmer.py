...
def _build_per_kmer_hist_figure(
    ...
    titles = [
        f"{decode_kmer(int(k))}  n={int(n_total[k])}  above={above_rate[k] * 100:.2f}%" for k in idx
    ]

    fig = make_subplots(
        rows=rows, cols=3, subplot_titles=titles, horizontal_spacing=0.06, vertical_spacing=0.12
    )
    ...
    for j, k in enumerate(idx):
        ...
        y_pred = 1.0 / (sg_p * np.sqrt(2 * np.pi)) * np.exp(-0.5 * ((x_smooth - mu_p) / sg_p) ** 2)
        fig.add_trace(
            go.Scatter(
                x=x_smooth,
                y=y_pred,
        ...
        y_obs = 1.0 / (sg_o * np.sqrt(2 * np.pi)) * np.exp(-0.5 * ((x_smooth - mu_o) / sg_o) ** 2)
        fig.add_trace(
            go.Scatter(
                x=x_smooth,
                y=y_obs,
        ...
        fig.update_xaxes(
            title_text="IPD" if r == rows else "", range=[0, max(60, mu_p + sg_p * 5)], row=r, col=c
        )
    ...
    fig.update_layout(
        ...
        title={"text": "Per-kmer empirical IPD vs AI prediction", "x": 0.5, "xanchor": "center"},
        bargap=0.0,
...
def build_figure(d, derived, top_k: int, min_obs: int, pattern: str | None):
    ...
    if pattern:
        ...
        log.info(
            "Pattern '%s' matches %d kmers (of %d with n≥%d)",
            pattern,
            cand.size,
            idx_pool.size,
            min_obs,
    ...
    fig.update_layout(
        ...
        title={
            "text": (
                f"Per-kmer outliers vs AI baseline — "
...
def main(argv=None):
    ...
    p.add_argument(
        "output_dir", help="Directory containing per_kmer_observed.npz (from the per-kmer step)."
    )
    p.add_argument(
        "--top-k",
        type=int,
        default=30,
        help="Number of top kmers to show in the detail panel (default 30).",
    )
    p.add_argument(
        "--min-obs",
        type=int,
    ...
    log.info(
        "  n_kmers covered (>=%d obs): %d",
        args.min_obs,
        int((derived["n_total"] >= args.min_obs).sum()),
    )
