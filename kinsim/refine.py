...
def slowed_split(
    ...
    if pooled_for_stats and kmer_baseline_means:
        ...
        log.info("[refine] baseline IPD per-sample:  n=%d  mean=%.2f  std=%.2f",
                 len(pooled), float(pooled.mean()), float(pooled.std()))
        log.info("[refine]   per-sample quantiles:    "
                 ...
                 float(pooled.max()))
        log.info("[refine] baseline PER-KMER MEAN: n_kmers=%d (>= %d samples each)  "
                 "mean=%.2f  std=%.2f",
                 len(kmer_means), MIN_BASELINE_PER_KMER_FOR_THRESHOLD,
                 float(kmer_means.mean()), float(kmer_means.std()))
        log.info("[refine]   per-kmer-mean quantiles: "
        ...
        log.info("[refine] threshold = p%g(baseline PER-KMER MEAN) = %.2f  "
    ...
    log.info("[refine] baseline:  %d in -> %d kept (pass-through)",
             n_baseline_in, n_baseline_out)
    log.info("[refine] near_meth: %d in -> %d kept (pass-through)",
             n_near_in, n_near_out)
    log.info("[refine] slowed:    %d in -> %d kept, %d dropped (IPD < %.2f)  "
             "survival = %.2f%%",
             n_slowed_in, n_slowed_kept, n_slowed_dropped, threshold,
    ...
    if slowed_kept_ipds:
        ...
        if len(sp) > 0:
            log.info("[refine] slowed kept quantiles:  "
...
def refine_pkl(
    ...
    from kinsim_config.yaml (typically 95).
    """
    ...
    # Resolve threshold percentile from config if not given.
    ...
    if not v4_data:
        log.error("No int-keyed data found — input is not in v4 format.")
        sys.exit(1)

    new_data, stats = slowed_split(v4_data, secondary_percentile)
...
def main(argv=None):
    ...
    ap.add_argument("--seed", type=int, default=42,
                    help="Random seed (currently unused but reserved).")
