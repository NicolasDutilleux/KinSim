...
class GFFLabeler(BaseLabeler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(qv_threshold=qv_threshold, file_pattern=file_pattern, **kwargs)
        ...
        # → dict[ref_id, list[(pos_0based, meth_id, strand)]].
    ...
    def _parse_file(
        ...
        with open(path) as f:
            for line in f:
                ...
                n_kept += 1
        log.info(
            "[GFFLabeler] %s parsed once  kept=%d  skipped(qv<%g)=%d  skipped(type)=%d",
            path.name, n_kept, self.qv_threshold, n_skipped_qv, n_skipped_type,
        )
