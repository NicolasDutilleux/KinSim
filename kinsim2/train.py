...
_N_KINETIC_CHANNELS = 4  # bilateral v2: fi, fp, ri, rp
...
def _gaussian_nll_loss(
    ...
    """Gaussian NLL for (fi, fp, ri, rp) jointly.

    Args:
        params:        Model output (B, 8) — [4×μ, 4×logσ].
        targets:       Ground-truth (B, 4) — log1p(fi, fp, ri, rp).
        log_sigma_min: Lower clamp on log σ.
        log_sigma_max: Upper clamp on log σ.
    """
    mu = params[:, :_N_KINETIC_CHANNELS]
    log_sig = torch.clamp(params[:, _N_KINETIC_CHANNELS:], log_sigma_min, log_sigma_max)
...
def _beta_nll_loss(
    ...
    NLL by ``σ²ᵝ`` (detached from the graph). β=0.5 default.
    """
    mu = params[:, :_N_KINETIC_CHANNELS]
    log_sig = torch.clamp(params[:, _N_KINETIC_CHANNELS:], log_sigma_min, log_sigma_max)
    var = torch.exp(2.0 * log_sig)
...
def _per_sample_gaussian_nll(
    ...
    """Per-sample GNLL on the 4 kinetic channels. Returns (N,) array."""
...
def _mse_loss(params: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    """Plain MSE on the 4 mean channels."""
    return nn.functional.mse_loss(params[:, :_N_KINETIC_CHANNELS], targets)


def _huber_loss(params: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    """Huber (smooth L1) loss on the 4 mean channels."""
    return nn.functional.huber_loss(params[:, :_N_KINETIC_CHANNELS], targets)
...
def _compute_metrics(
    ...
    """Compute overall + per-meth-type metrics on bilateral 4-channel output.

    Channel layout: 0=ipd_fwd, 1=pw_fwd, 2=ipd_rev, 3=pw_rev. IPD metrics
    aggregate over fwd+rev; PW metrics do likewise.
    """
    ...
    mse_ipd = float((mse[0] + mse[2]) / 2.0)
    mse_pw = float((mse[1] + mse[3]) / 2.0)
    result.update(
        {
            f"{prefix}_mse_ipd": mse_ipd,
            f"{prefix}_mse_pw": mse_pw,
            f"{prefix}_mse_mu": float((mse_ipd + mse_pw) / 2.0),
            f"{prefix}_mae_ipd": float((mae[0] + mae[2]) / 2.0),
            f"{prefix}_mae_pw": float((mae[1] + mae[3]) / 2.0),
            f"{prefix}_pearson_ipd": _pearson(
                all_mu[:, ipd_cols].ravel(), all_true[:, ipd_cols].ravel()
            ),
            f"{prefix}_pearson_pw": _pearson(
                all_mu[:, pw_cols].ravel(), all_true[:, pw_cols].ravel()
            ),
            f"{prefix}_calib_1sig_ipd": float((in_1sig[0] + in_1sig[2]) / 2.0),
            f"{prefix}_calib_1sig_pw": float((in_1sig[1] + in_1sig[3]) / 2.0),
            f"{prefix}_calib_2sig_ipd": float((in_2sig[0] + in_2sig[2]) / 2.0),
            f"{prefix}_calib_2sig_pw": float((in_2sig[1] + in_2sig[3]) / 2.0),
            f"{prefix}_calib_3sig_ipd": float((in_3sig[0] + in_3sig[2]) / 2.0),
            f"{prefix}_calib_3sig_pw": float((in_3sig[1] + in_3sig[3]) / 2.0),
        }
    )
    ...
    for meth_id in sorted(np.unique(all_meth_ids)):
        ...
        by_type[name] = {
            "n": int(mask.sum()),
            "pearson_ipd": _pearson(
                mu_m[:, ipd_cols].ravel(), true_m[:, ipd_cols].ravel()
            ),
            "pearson_pw": _pearson(
                mu_m[:, pw_cols].ravel(), true_m[:, pw_cols].ravel()
            ),
            "mae_ipd": float(np.abs(diff_m[:, ipd_cols]).mean()),
            "mae_pw": float(np.abs(diff_m[:, pw_cols]).mean()),
    ...
    var_mu_ipd = float(np.var(all_mu[:, ipd_cols]))
    var_mu_pw = float(np.var(all_mu[:, pw_cols]))
    e_sig2_ipd = float(np.mean(all_sigma[:, ipd_cols] ** 2))
    e_sig2_pw = float(np.mean(all_sigma[:, pw_cols] ** 2))
    self_oracle_ipd = (
    ...
    if kmer_ids is not None and len(kmer_ids) == len(all_true):
        ...
        _, inv, counts = np.unique(bucket_keys, return_inverse=True, return_counts=True)
        keep_mask = counts[inv] >= _ORACLE_MIN_PER_BUCKET
        if keep_mask.any():
            # Pool IPD across fwd+rev, PW across fwd+rev — bilateral oracle.
            ipd_pooled = all_true[keep_mask][:, ipd_cols].mean(axis=1)
            pw_pooled = all_true[keep_mask][:, pw_cols].mean(axis=1)
            denom = np.maximum(np.bincount(inv[keep_mask]), 1)
            mu_true_ipd_per = np.bincount(inv[keep_mask], weights=ipd_pooled) / denom
            mu_true_pw_per = np.bincount(inv[keep_mask], weights=pw_pooled) / denom
            sqr_ipd_per = np.bincount(inv[keep_mask], weights=ipd_pooled ** 2) / denom
            sqr_pw_per = np.bincount(inv[keep_mask], weights=pw_pooled ** 2) / denom
    ...
    for meth_id in sorted(np.unique(all_meth_ids)):
        ...
        mu_sel = all_mu[mask][idx]  # (n_pick, 4)
        sig_sel = all_sigma[mask][idx]  # (n_pick, 4)
        name = _meth_names().get(int(meth_id), f"meth{meth_id}")
        dist_samples[name] = {
            "mu_ipd": mu_sel[:, ipd_cols].mean(axis=1),
            "mu_pw": mu_sel[:, pw_cols].mean(axis=1),
            "sigma_ipd": sig_sel[:, ipd_cols].mean(axis=1),
            "sigma_pw": sig_sel[:, pw_cols].mean(axis=1),
        }
...
def _bucket_keys_from_arrays(categories: np.ndarray) -> np.ndarray:
    """Map per-sample category to a bucket label.

    Bilateral v2 retains only the category in the per-row payload (parent
    meth + offset live in the shard but are not loaded into batches). The
    bucket view collapses to three rows: BASELINE / SLOWED / NEAR_METH.
    """
    keys = np.empty(len(categories), dtype=object)
    keys[:] = "OTHER"
    keys[categories == 0] = "BASELINE"
    keys[categories == 1] = "SLOWED"
    keys[categories == 2] = "NEAR_METH"
    return keys
...
def _compute_bucket_metrics(
    ...
    bucket_keys = _bucket_keys_from_arrays(all_categories)
    ...
    for bk in np.unique(bucket_keys):
        ...
        rec = {
            ...
            "pearson_ipd": _pearson(
                mu_b[:, ipd_cols].ravel(), true_b[:, ipd_cols].ravel()
            ),
            "pearson_pw": _pearson(
                mu_b[:, pw_cols].ravel(), true_b[:, pw_cols].ravel()
            ),
            "mae_ipd": float(np.abs(diff_b[:, ipd_cols]).mean()),
            "mae_pw": float(np.abs(diff_b[:, pw_cols]).mean()),
...
def _bucket_sort_key(bk: str) -> tuple:
    """Sort: BASELINE, SLOWED, NEAR_METH, OTHER."""
    order = {"BASELINE": 0, "SLOWED": 1, "NEAR_METH": 2}
    return (order.get(bk, 9), bk)
...
class KineticPredictor(L.LightningModule):
    ...
    def __init__(
        ...
        # Bilateral v2 no longer emits parent_meth / parent_offset — bucket
        # metrics group on category alone (parent is implicit in the shard).
    ...
    def forward(
        ...
        return self.model(kmer_ids, meth_ctx_fwd, meth_ctx_rev)
    ...
    def _unpack_batch(self, batch: tuple) -> tuple:
        """Bilateral 6-tuple: (kmer_ids, mc_fwd, mc_rev, targets, cat_fwd, cat_rev)."""
        return batch

    def training_step(self, batch: tuple, batch_idx: int) -> torch.Tensor:
        kmer_ids, mc_fwd, mc_rev, signals, *_extras = self._unpack_batch(batch)
        loss = self._loss_fn(
            self.model(kmer_ids, mc_fwd, mc_rev),
            signals,
            **self._clamp_kwargs(),
        )
    ...
    def validation_step(self, batch: tuple, batch_idx: int) -> torch.Tensor:
        kmer_ids, mc_fwd, mc_rev, signals, cat_fwd, cat_rev = self._unpack_batch(batch)
        params = self.model(kmer_ids, mc_fwd, mc_rev)
        clamp = self._clamp_kwargs()
        loss = self._loss_fn(params, signals, **clamp)
        self.log("val_loss", loss, on_step=False, on_epoch=True, prog_bar=True)
        mu = params[:, :_N_KINETIC_CHANNELS]
        log_sig = torch.clamp(
            params[:, _N_KINETIC_CHANNELS:],
            clamp["log_sigma_min"],
            clamp["log_sigma_max"],
        )
        sigma = torch.exp(log_sig)
        # Derive per-row meth_id from category_fwd as a proxy (0=baseline,
        # 1=slowed, 2=near). Real per-meth-type breakdown needs parent_meth
        # which the bilateral dataset no longer emits — bucket metrics
        # below carry the meth-resolved view.
        meth_ids = cat_fwd.clamp(min=0)
        ...
        self._val_categories.append(cat_fwd.detach().cpu())
    ...
    def on_validation_epoch_end(self) -> None:
        if not self._val_mu:
            return
        all_mu = torch.cat(self._val_mu).numpy()  # (N, 4)
        all_sigma = torch.cat(self._val_sigma).numpy()  # (N, 4)
        all_true = torch.cat(self._val_true).numpy()  # (N, 4)
    ...
    def test_step(self, batch: tuple, batch_idx: int) -> None:
        kmer_ids, mc_fwd, mc_rev, signals, cat_fwd, _cat_rev = self._unpack_batch(batch)
        params = self.model(kmer_ids, mc_fwd, mc_rev)
        clamp = self._clamp_kwargs()
        mu = params[:, :_N_KINETIC_CHANNELS]
        log_sig = torch.clamp(
        ...
        self._test_categories.append(cat_fwd.detach().cpu())
    ...
    def sample(
        ...
        return self.model.sample(kmer_ids, meth_ctx_fwd, meth_ctx_rev)
...
def _infer_num_meth_types_from_data(pkl_path: str) -> int:
    ...
    for i, path in enumerate(paths, 1):
        ...
        raw_params = (meta or {}).get("extraction_params") or {}
        try:
            from .utils.config import ExtractionParams
            layout = get_sample_layout(ExtractionParams.from_dict(raw_params))
        except (ValueError, TypeError, KeyError):
            from .utils.config import get_extraction_params
            layout = get_sample_layout(get_extraction_params())
        for k, arr in data.items():
            if not isinstance(k, (int, np.integer)) or not isinstance(arr, np.ndarray):
                continue
            if arr.size == 0 or arr.shape[1] != layout.n_cols:
                continue
            mc_fwd_max = int(
                arr[:, layout.col_meth_ctx_fwd_start : layout.col_meth_ctx_fwd_end].max()
            )
            mc_rev_max = int(
                arr[:, layout.col_meth_ctx_rev_start : layout.col_meth_ctx_rev_end].max()
            )
            pm_fwd_max = int(arr[:, layout.col_parent_meth_fwd].max())
            pm_rev_max = int(arr[:, layout.col_parent_meth_rev].max())
            shard_max = max(mc_fwd_max, mc_rev_max, pm_fwd_max, pm_rev_max)
            if shard_max > max_id:
                max_id = shard_max
...
def _save_model_config(
    ...
    # Provenance: lets a reviewer reconstruct the exact code that produced
    # this checkpoint. "unknown" when not under a git checkout.
    try:
...
def train_main(
    ...
    if shard_params is None:
        ...
        log.info(
            ...
            "kmer_size=%d  upstream=%d  downstream=%d",
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Global determinism: torch + numpy + Python random + Lightning DataLoader
    # workers. Same shard set + same hyperparameters reproduce bit-identical weights.
