...
def motif_contains(longer: str, offset_longer: int, shorter: str, offset_shorter: int) -> bool:
    ...
    for start in range(len_l - len_s + 1):
        if start + offset_shorter != offset_longer:
            continue
        if all(_iupac_compatible(longer[start + i], shorter[i]) for i in range(len_s)):
            return True
    return False
...
def _parse_combined_rows(reader: csv.DictReader, filepath: str) -> list[dict]:
    ...
    for lineno, row in enumerate(reader, 2):
        ...
        if norm_mod is None:
            log.warning("%s line %d: unknown mod_type '%s' -- skipped", filepath, lineno, mod_raw)
            continue
        ...
        try:
            ...
            log.warning("%s line %d: invalid numeric field -- skipped", filepath, lineno)
            continue
    ...
    for lineno, row in enumerate(reader, 2):
        ...
        if not norm_mod:
            if mod_raw in ("modified_base", ""):
                ...
                log.warning(
                    "%s line %d: unknown modificationType '%s' -- skipped",
                    filepath,
                    lineno,
                    mod_raw,
                )
...
def _apply_filters(
    ...
    for e in entries:
        ...
        if isinstance(frac, float) and frac < min_frac:
            log.info(
                "  [FILTERED] %s -- fraction=%.3f < min_frac=%.2f  (source: %s)",
                label,
                frac,
                min_frac,
                e.get("source", "?"),
        ...
        if isinstance(count, int) and min_sites > 0 and count < min_sites:
            log.info(
                "  [FILTERED] %s -- count=%d < min_sites=%d  (source: %s)",
                label,
                count,
                min_sites,
                e.get("source", "?"),
        ...
        if min_detected > 0 and isinstance(n_detected, int) and n_detected < min_detected:
            log.info(
                "  [FILTERED] %s -- nDetected=%d < min_detected=%d  (source: %s)",
                label,
                n_detected,
                min_detected,
                e.get("source", "?"),
        ...
        log.info(
            "  [KEPT]     %s  count=%s%s",
            label,
            str(count) if isinstance(count, int) else "N/A",
            bypass_note,
        )
        kept.append(e)
    ...
    log.info(
        "Filtering summary: %d in -> %d kept, %d dropped "
...
def deduplicate_motifs(entries: list[dict]) -> list[dict]:
    ...
    for mod_type, motifs in sorted(by_mod.items()):
        ...
        log.info(
            "  %s: %d -> %d motifs after deduplication (%d redundant removed)",
            mod_type,
            n,
            len(kept),
            n_removed,
...
def _merge_entries(a: list[dict], b: list[dict]) -> list[dict]:
    ...
    if matched:
        ...
        for key in sorted(matched):
            e = seen[key]
            src = e.get("source", "?")
            log.info("  [MATCH]    %s %s offset=%d  (kept from: %s)", key[0], key[1], key[2], src)
    ...
    if only_in_a:
        ...
        for key in sorted(only_in_a):
            e = seen[key]
            src = e.get("source", "?")
            log.info("  [UNIQUE]   %s %s offset=%d  (source: %s)", key[0], key[1], key[2], src)
    ...
    log.info(
        "Merge: %d + %d -> %d unique motifs (%d matched across files)",
        len(a),
        len(b),
        len(seen),
        len(matched),
...
def merge_motifs(
    ...
    *,
    ...
    log.info(
        "  Filters: min_frac=%.2f, min_sites=%d, min_detected=%d, dedup=%s",
        min_frac,
        min_sites,
        min_detected,
        deduplicate,
    ...
    filtered = _apply_filters(
        merged, min_frac=min_frac, min_sites=min_sites, min_detected=min_detected
    )
    ...
    log.info(
        "  After filtering:       %d  (%d removed)",
        motifs_after_filter,
        motifs_in - motifs_after_filter,
    )
    log.info(
        "  After deduplication:   %d  (%d redundant removed)",
        motifs_out,
        motifs_after_filter - motifs_out,
    )
...
def main(argv=None) -> None:
    ...
    print(
        f"After filtering:     {n_filt:>6}  ({pct_f:.1f}% retained,"
